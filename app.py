...
def handle_message(data): # Empfängt Daten über WebSocket und führt anschließend eine POST-Anfrage aus
    ...
    chat_history.append(AIMessage(content=response))  # KI-Antwort zur Chat-Historie hinzufügen
    
    # emit response
    emit('response', {'message': response})  # Sende die generierte KI-Antwort über WebSocket zurück an den Client, damit sie im Frontend angezeigt werden kann
